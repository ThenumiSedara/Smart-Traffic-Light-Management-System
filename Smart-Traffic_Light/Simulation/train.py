...
# Read vehicle counts from JSON file
with open("../Smart-Traffic_Light//Object-Detection/object_counts.json", "r") as f:
    vehicle_counts = json.load(f)

def generate_vehicles_in_sumo(vehicle_counts):
    # Iterate over junctions and vehicle counts
    for junction, count in vehicle_counts.items():
        # Generate vehicles for each junction based on the count
        for i in range(count):
            # Example: Generate vehicle in SUMO
            traci.vehicle.add(vehicleID=f"vehicle_{i}_junction_{junction}", routeID=f"route_{junction}")
...
def run(train=True, model_name="model", epochs=50, steps=500, ard=False):
    ...
    traci.close()
    # Generate vehicles in SUMO
    generate_vehicles_in_sumo(vehicle_counts)
